...
export default function Canvas() {
  ...
  const { emitAwareness, onAwareness } = useCollab();
  ...
  useEffect(() => {
    return () => {
      ...
      emitAwareness({ cursor: null, selectedTableId: null, linking: null });
    };
  }, [emitAwareness]);
  ...
  const handlePointerDownOnElement = (e, { element, type }) => {
    ...
    if (!element.locked || !(e.ctrlKey || e.metaKey)) {
      ...
      emitAwareness({ selectedTableId: element.id });
    }
    ...
    if (e.ctrlKey || e.metaKey) {
      if (isSelected) {
        if (bulkSelectedElements.length > 1) {
          ...
          emitAwareness({ selectedTableId: null });
        }
      } else {
  ...
  const handlePointerMove = (e) => {
    ...
    if (!rafRef.current) {
      rafRef.current = requestAnimationFrame(() => {
        rafRef.current = null;
        emitAwareness({ cursor: cursorPosRef.current });
      });
    }
    ...
    <div className="grow h-full touch-none" id="canvas">
      ...
      >
        ...
        >
          ...
          {tables
            .filter((table) =>
              Object.values(remoteCursors).some(
                (u) => u.selectedTableId === table.id
              )
            )
            .map((table) => (
              <rect
                key={`remote-selected-${table.id}`}
                x={table.x}
                y={table.y}
                width={settings.tableWidth}
                height={getTableHeight(table, settings.tableWidth, settings)}
                fill="none"
                stroke="#00d4aa"
                strokeWidth={2}
                strokeDasharray="6 3"
          ...
          {Object.entries(remoteCursors).map(([socketId, user]) => {
            if (!user.cursor) return null;
            const color = getColor(user.username);
            return (
              <g key={socketId} className="remote-cursor" style={{ pointerEvents: "none" }}>
                <circle cx={user.cursor.x} cy={user.cursor.y} r={5}
                  fill={color} stroke="#fff" strokeWidth={1.5} />
                <text x={user.cursor.x + 9} y={user.cursor.y + 4}
