...
export default function WorkSpace({ forcedDiagramId } = {}) {
  ...
  const save = useCallback(async () => {
    ...
    if (isTemplate || (!loadedDiagramId && !isTemplate && !isDiagram)) {
      const diagram = await apiCreateDiagram(
        title, database,
        { database, name: title, tables, references: relationships, notes, areas, pan: transform.pan, zoom: transform.zoom, ...(databases[database].hasEnums && { enums }), ...(databases[database].hasTypes && { types }) },
      );
  ...
  const load = useCallback(async () => {
    ...
    const loadTemplate = async (id) => {
      ...
      try {
        ...
        template = await db.templates.where("templateId").equals(id).first();
      } catch (e) {
  ...
  useEffect(() => {
    document.title = "Editor | DBDesign";
  ...
  useEffect(() => {
    ...
    if (socket) {
      socket.emit("join-room", { diagramId: loadedDiagramId });
    }
  }, [loadedDiagramId, cloudOnly]);
